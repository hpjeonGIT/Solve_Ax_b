...
int main(int argc, char **argv)
{
   ...
   /*   0    0    0    0    0    0    8   10    8    0
    ...
    // update of matrix
    if (myid == 0){
        data = {10., 7., 7., 10., 7., 7., 10., 7., 7., 10., 7.};
	AMGX_matrix_replace_coefficients(A, n_global, nnz, data.data(), 0);
    } else if (myid == 1) {
        data = {7., 10., 7., 7., 10., 7., 7., 10., 7.};
	AMGX_matrix_replace_coefficients(A, n_global, nnz, data.data(), 0);
    } else {// myid == 2
	data = {7., 10., 7., 7., 10., 7., 7., 10};
	AMGX_matrix_replace_coefficients(A, n_global, nnz, data.data(), 0);
    }
    AMGX_solver_setup(solver, A);
    AMGX_solver_solve(solver, b, x);
    ...
    for (int i=0; i < local_size; i++)
            std::cout << " " << h_x[i];
    std::cout << std::endl;
    // When vector b comes from device memory
    double *d_b = nullptr;
    cudaMalloc((void**)&d_b, h_b.size() * sizeof(double));
    cudaMemcpy(d_b, h_b.data(), h_b.size() * sizeof(double),
	       cudaMemcpyHostToDevice);
    AMGX_vector_upload(x, local_size, 1, &h_x[0]);
    AMGX_vector_upload(b, local_size, 1, &d_b[0]);
    AMGX_solver_setup(solver, A);
    AMGX_solver_solve(solver, b, x);
    AMGX_solver_get_status(solver, &status);
    AMGX_vector_download(x, &h_x[0]);
